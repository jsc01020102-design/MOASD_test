...
import { 
  Video, FileText, Play, ExternalLink, Award, Sparkles,
        ...
        {selectedVideo && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="w-full max-w-4xl bg-slate-900 border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col text-left"
            >
              {/* Header inside modal */}
              <div className="flex justify-between items-center px-6 py-4 border-b border-white/5">
                <div className="flex items-center gap-2 text-cyan-400">
                  ...
                  <h4 className="text-sm font-bold text-white tracking-tight">
                    {language === 'en' ? selectedVideo.titleEn : selectedVideo.title}
                  </h4>
                </div>
                <button 
                  onClick={() => setSelectedVideo(null)}
                  className="p-1.5 text-slate-400 hover:text-white hover:bg-white/5 rounded-full transition-all cursor-pointer border-0 bg-transparent"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {/* Details & Metadata below video */}
              <div className="p-6 bg-slate-950 space-y-4">
                <p className="text-xs text-slate-300 leading-relaxed max-h-32 overflow-y-auto">
                  {language === 'en' ? selectedVideo.descriptionEn : selectedVideo.description}
                </p>

                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-white/5 text-[11px] font-mono text-slate-500">
                  <div className="flex flex-wrap gap-x-4 gap-y-1">
                    <div>
                      <span>{language === 'en' ? 'Resolution: ' : '인식 등급: '}</span>
                      <span className="text-slate-300">Auto Ultra HD (HDR)</span>
                    </div>
                    <div>
                      <span>{language === 'en' ? 'Format: ' : '연동 규격: '}</span>
                      <span className="text-slate-300">
                        {getYoutubeId(selectedVideo.videoUrl) ? 'YouTube Stream' : 'Direct MP4 File'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <a 
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-cyan-950/40 hover:bg-cyan-900/40 border border-cyan-500/20 rounded-lg text-[10px] font-bold text-cyan-400 transition-colors uppercase cursor-pointer border-0"
                    >
                      <ExternalLink className="w-3.5 h-3.5" />
                      <span>{language === 'en' ? 'Open Original Link' : '원본 링크로 시청'}</span>
                    </a>
                  </div>
                </div>
              </div>
